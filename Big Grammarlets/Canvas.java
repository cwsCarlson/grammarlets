...
public class Canvas extends World
{
    ...
    {    
        ...
        writeText(key.nextQuestion(), 49);
        undoButton = new Button("undo"); //Create the undo and submit buttons.
        addObject(undoButton, 727, 494);
        submitButton= new Button("idk");
        addObject(submitButton, 678, 555);
    }
    
    /**
     * Checks for button presses and mouse drags, and world updates every act cycle.
     */
    public void act()
    {
     ...
     */
    public void writeText(String allText, int charactersPerLine)
    {
        ...
        for(int i = 0; i < text.length; i++)
        {
            ...
            if(currentChars > charactersPerLine) //If too high, move current word to next line and start over
            {
                line++;
            ...
            GreenfootImage image = new GreenfootImage(text[i], 40, Color.BLACK, Color.WHITE); //create the Word's image, which is the text
    ...
    /**
     * Calculates the y-position of a new brace.
     */
    public int calculateBracePos(int yPos)
    {
        for(int i = 0; i < fullText.length; i++) //check all lines
        {
            if(yPos > getLinePos(i) && yPos < getLinePos(i + 1)) //if below this line and above the next
                return getLinePos(i) + 40;
        }
        return 10; //if brace (somehow) isn't on the screen
    }
    
    /**
     * Calculates the y-position of a line of text.
     */
    public int getLinePos(int line)
    {
        return 65 + (75 * line); //each line is 50 apart
    }
    
    /**
     * Returns the words in a line based on the y-position of a brace
     */
    public Actor[] getWordsInLine(int yPos)
    {
        int line = (yPos - 70) / 75; //brace is 70 below line start
        return fullText[line];
    }
    
    /**
     * Places a new strikethrough, correction, and reasoning on the canvas.
     */
    public void placeCorrection(String toCover, String replace, int reason)
    {
        ...
        strikes[strikeNo] = new Strike(initialX, currentWords[0].getY(), finalX, key.checkSelection(selected)); //create a new strikethrough
        addObject(strikes[strikeNo], (initialX + finalX) / 2, currentWords[0].getY() + 5);

        corrections[strikeNo] = new Word(replace); //create new correction
        GreenfootImage image = new GreenfootImage(replace, 18, getTextColor(key.checkCorrection(replace)), Color.WHITE); //create text
        corrections[strikeNo].setImage(image); //set text 
        addObject(corrections[strikeNo],(initialX + finalX) / 2, currentWords[0].getY() - 20); //add correction
        
        reasons[strikeNo] = new Word(reasonList[reason - 1]); //create new reason
        image = new GreenfootImage(reasonList[reason - 1], 18, getTextColor(key.checkReason(reason, selected)), Color.WHITE);
        reasons[strikeNo].setImage(image); //add reason text
        addObject(reasons[strikeNo],(initialX + finalX) / 2, currentWords[0].getY() - 40); //add reason
        
        strikeNo++; //increase strike number
    }
    
    /**
     * Passes the green and red shades used to mark correct and incorrect answers.
     */
    ...
    public void submitAnswer()
    {
        ...
        reasons = new Actor[10];
        strikeNo = 0;
        undoButton = new Button("undo"); //re-add the buttons
        addObject(undoButton, 727, 494);
        submitButton= new Button("idk");
        addObject(submitButton, 678, 555);
